...
1. **Authenticate as Operator** (step_page)
   - Operator logs in or signs up, optionally using an invite code, and is validated for operator permissions.
   - Page type: Standard page

2. **Choose to Sign Up or Browse as Guest** (decision)
   - User decides whether to sign up/login or start browsing listings without an account.

3. **Authenticate (Sign Up or Login)** (step_page)
   - Seller accesses authentication page to sign up or log in, selecting the seller role.
   - Page type: Standard page

4. **Authenticate as Admin** (step_page)
   - Admin logs in via secure form, with optional two-factor authentication and role validation.
   - Page type: Standard page

5. **Complete Seller Onboarding** (step_page)
   - Seller completes onboarding: profile info, KYC/verification, Stripe Connect setup, and first listing wizard.
   - Page type: Standard page

6. **Authenticate (Sign Up or Login)** (step_page)
   - User accesses the authentication page to sign up with email/social or log in; validation and error handling are enforced.
   - Page type: Standard page

7. **View Dashboard & Order Status** (step_page)
   - User accesses their dashboard to view ongoing and past orders, order status, and actionable next steps (cancel, dispute, review).
   - Page type: Standard page
...
- [ ] User flows complete: Authenticate as Operator, Choose to Sign Up or Browse as Guest, Authenticate (Sign Up or Login), Authenticate as Admin, Complete Seller Onboarding, Authenticate (Sign Up or Login), View Dashboard & Order Status
